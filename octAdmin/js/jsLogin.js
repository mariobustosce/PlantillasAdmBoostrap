function recuperarClave() {
	if($("#txt_rut").val()!=''){
		...
		if(rut.match("-")){
			if(rut.length<=12){
				$.ajax({
		            url: "http://localhost/audisis/login/recuperarClave",
		            method: "POST",
		            ...
		            success: function(data) {
		            	if(data.match("OP1")){
		            		...
		            		window.location = "http://localhost/audisis/login/success";
		            	}else{	
	...
	if($("#txt_usuario1").val()!="" && $("#txt_usuario2").val()!=""){
		...
		if(pass1==pass2){
			if (pass1.length>=6) {
				...
				$.ajax({
		            url:"http://localhost/audisis/App_ModuloPerfilUsuario/ModificarContrasenia", 
		            method: "POST",
		            data: "pass="+pass2+"&mail="+email+"&usuario="+idUsu,
		            success: function(data) {
		            	if(data.match("1")){
		            		window.location = "http://localhost/audisis/login/change";
		            	}else{	
		            		alertify.error("Error Desconocido");
		            	}
		            }
